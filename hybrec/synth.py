...
__all__ = ['SynthDataset', 'gen_values', 'gen_added_n_deleted', 'exclude_element', 'build_interactions',
           'build_features_from_df', 'df_highlight_interaction', 'df_color_positive_interaction']
...
class SynthDataset():
    ...
    def __init__(self):
        ...
        self.deleted_user_features = []
        self.added_user_features = []
        self.deleted_item_features = []
        self.added_item_features = []
...
# Cell
def build_features_from_df(feature_interactions_df:pd.DataFrame, element_id_column:str, feature_column:str, tolist=True) -> [tuple]:
    '''
...
def add_user_metadata(self: SynthDataset):
    ...
    '''
    users_features = build_features_from_df(
        build_interactions(
            all_users,
            all_user_features,
            l1_col_name='user_id',
...
def add_item_metadata(self: SynthDataset):
    ...
    '''
    items_features = build_features_from_df(
        build_interactions(
            all_items,
            all_item_features,
            l1_col_name='item_id',
...
def build_synth_dataset(self:SynthDataset,
    ...
    if add_user_metadata:
        self.added_user_features, self.deleted_user_features = gen_added_n_deleted(self.all_user_features, max_added=max_added, max_deleted=max_deleted)
        if print_added_n_deleted: print('added user features: {}\t deleted user features: {}'.format(self.added_user_features, self.deleted_user_features))

    if add_item_metadata:
        self.added_item_features, self.deleted_item_features = gen_added_n_deleted(self.all_item_features, max_added=max_added, max_deleted=max_deleted)
        if print_added_n_deleted: print('added item features: {}\t deleted item features: {}'.format(self.added_item_features, self.deleted_item_features))
